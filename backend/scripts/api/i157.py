"""
Indicateur i157 : Nombre de licenciés sportifs pour 100 habitants 2022

Source : observatoire des territoires
URL : https://www.observatoire-des-territoires.gouv.fr/outils/cartographie-interactive/#c=indicator&i=licsport.p_licsport&s=2022&view=map78
...
def fetch_raw_csv(filename: str, sep=";", header=2) -> pd.DataFrame:
    ...
    csv_path = script_dir.parent / "source" / filename  # scripts/source/i157.csv
...
def clean_and_prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    df = (df.rename(
            columns={"Code": "epci_id",
                    "Nombre de licenciés sportifs pour 100 habitants 2022": "value",})
...
def run(csv_filename: str) -> None:
    ...
    try:
        ensure_indicator_exists(session, "i157") # adapter avec l'indicateur_id
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description= "Import CSV -> valeur_indicateur (indicateur i157)") # "Import CSV -> valeur_indicateur"
    parser.add_argument("--csv", default= "i157.csv", help= "Nom du fichier CSV à importer (dans scripts/source/)",) # adapter le default
    parser.add_argument(
        "--dry-run",
        action="store_true",
