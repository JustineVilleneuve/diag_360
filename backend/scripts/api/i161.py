"""
Indicateur i161 : Indice de vieillissement 2022
...
def fetch_raw_csv(filename: str, sep=";", header=2) -> pd.DataFrame:
    ...
    csv_path = script_dir.parent / "source" / filename  # scripts/source/i161.csv
...
def clean_and_prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.rename(
        columns={
            "Code": "epci_id",
            "Indice de vieillissement 2022": "value",
        }
...
def run(csv_filename: str) -> None:
    ...
    try:
        ensure_indicator_exists(session, "i161")  # adapter avec l'indicateur_id
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import CSV -> valeur_indicateur (indicateur i161)"
