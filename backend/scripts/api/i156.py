"""
Indicateur i156 : Nombre d'établissements industriels à risque (classés Seveso) 2025

Source : observatoire des territoires
URL : https://www.observatoire-des-territoires.gouv.fr/outils/cartographie-interactive/#c=indicator&f=seveso_tot&i=seveso.nb_seveso&s=2025&view=map78 
Dernières données disponibles : 2025 
...
def clean_and_prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    df = (df.rename(
            columns={"Code": "epci_id",
                    "Nombre d'établissements industriels à risque (classés Seveso) 2025": "value",})
...
def run(csv_filename: str) -> None:
    ...
    try:
        ensure_indicator_exists(session, "i156") # adapter avec l'indicateur_id
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description= "Import CSV -> valeur_indicateur (indicateur i156)") # "Import CSV -> valeur_indicateur"
    parser.add_argument("--csv", default= "i156.csv", help= "Nom du fichier CSV à importer (dans scripts/source/)",) # adapter le default
    parser.add_argument(
        "--dry-run",
        action="store_true",
